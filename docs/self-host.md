...
2. Set `POSTGRES_PASSWORD` to letters and digits, such as the output of
   ...
   `ogremcp_postgres-data` volume, at the first start. To change it later, set
   it in Postgres first, then in `.env`, then run `docker compose up -d`:

   ```sh
   docker compose exec postgres psql -U ogremcp -d ogremcp -c '\password ogremcp'
   ```
3. Set `PUBLIC_BASE_URL` to the origin users reach, such as
   `https://ogremcp.example.com`. The default, `http://localhost:4790`, works
...
5. Generate the OAuth server's keys. These commands build the image and
   ...
   docker compose run --rm --no-deps -T ogremcp node platform/dist/gen-oidc-keys.js >> .env
   ```
...
Add `-d` to run it in the background. The `ogremcp` service waits until
Postgres is healthy, applies the pending migrations, and starts. When a
migration fails, the service does not start, and `docker compose logs ogremcp`
...
```sh
PUBLIC_BASE_URL=https://ogremcp.example.com
TRUST_PROXY_HOPS=1
```
...
`ogremcp-bridge/config.json` in the user's config folder, as `server_url`. The
...
- macOS, from a shell: quit the bridge, then run the app's binary. The
  ...
  ```sh
  "$HOME/Applications/Ogre MCP.app/Contents/MacOS/ogremcp-bridge" server set https://ogremcp.example.com
  ```
...
`OGREMCP_BASE_URL`, when set, overrides the setting. It is for development:
...
Postgres keeps its data in the `ogremcp_postgres-data` volume.

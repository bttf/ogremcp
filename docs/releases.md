...
- `release.prerelease: auto` does not work, because `--skip=validate` leaves the
  semver fields empty. Set `prerelease` explicitly.
...
- Build on macOS: the prototype's tray app needs cgo on macOS. The Windows
  build sets `CGO_ENABLED=0` and adds `-H windowsgui` to its ldflags.
...
- macOS signing, notarization, and the `.dmg` run in the post hook script (the
  ...
  are Pro.
- Windows code signing can run from `binary_signs`. Inno Setup needs Windows,

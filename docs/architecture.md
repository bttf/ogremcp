...
interface Interpreter<State> {
  /** Pure: no DB or network. Throws ParseError(userMessage, { adapterSchema?, flavor? }) on bad or unsupported input, with the facts it read before failing (§16.1). */
...
| Method | Path | Purpose |
|---|---|---|
| `GET` | `/api/v1/kits` | The user's enabled kits, each with its display name (e.g. "World of Warcraft", which the tray shows), manifest version, and adapter version. The adapter version is the addon TOC's `## Version` (semver); it equals the `addon_version` the adapter stamps (§6.3) and the `addon-v` tag (§5). |

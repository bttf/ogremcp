...
export function stateRules(experimental: readonly string[]): string[] {
  return [
    ...
    "Ground game facts in a source, never memory alone. Ogre MCP has no search tool: search the web yourself, preferring sources for the player's `flavor` over retail. The player's state is a source for what it says; search before saying where to go, who to see, where something is, or where an item comes from beyond that. If research is inconclusive, sources disagree, or you can't search, say how sure you are.",
